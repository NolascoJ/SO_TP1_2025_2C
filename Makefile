...
PLAYER_SRC := player/greedy_player.c
PLAYER2_SRC := player/cluster_player.c
...
TARGETS := $(BIN_DIR)/view $(BIN_DIR)/master $(BIN_DIR)/greedy $(BIN_DIR)/cluster
...
# Compilar el objeto de select_utils
...
# Jugador (greedy)
$(BIN_DIR)/greedy: $(PLAYER_SRC) $(PLAYER_LIB_SRC) $(SHM_OBJ) | $(BIN_DIR)
	$(CC) $(CFLAGS) -o $@ $(PLAYER_SRC) $(PLAYER_LIB_SRC) $(SHM_OBJ)

# Jugador variant 2 (cluster_player)
$(BIN_DIR)/cluster: $(PLAYER2_SRC) $(PLAYER_LIB_SRC) $(SHM_OBJ) | $(BIN_DIR)
...
valgrind-1p-view: all
	...
	TERM=$(TERM) $(VALGRIND) $(VG_FLAGS) --log-file=valgrind_1p_view_%p.log \
		$(BIN_DIR)/master -w 12 -h 12 -d 50 -t 3 -s 42 -v $(BIN_DIR)/view -p $(BIN_DIR)/greedy
...
valgrind-1p-noview: all
	...
	$(VALGRIND) $(VG_FLAGS) --log-file=valgrind_1p_noview_%p.log \
		$(BIN_DIR)/master -w 12 -h 12 -d 1 -t 3 -s 43 -p $(BIN_DIR)/greedy
...
valgrind-2p-same-view: all
	...
	TERM=$(TERM) $(VALGRIND) $(VG_FLAGS) --log-file=valgrind_2p_same_view_%p.log \
		$(BIN_DIR)/master -w 15 -h 15 -d 100 -t 4 -s 100 -v $(BIN_DIR)/view -p $(BIN_DIR)/greedy $(BIN_DIR)/greedy
...
valgrind-2p-mixed-view: all
	...
	TERM=$(TERM) $(VALGRIND) $(VG_FLAGS) --log-file=valgrind_2p_mixed_view_%p.log \
		$(BIN_DIR)/master -w 15 -h 15 -d 100 -t 4 -s 101 -v $(BIN_DIR)/view -p $(BIN_DIR)/greedy $(BIN_DIR)/cluster
...
valgrind-3p-mixed-view: all
	...
	TERM=$(TERM) $(VALGRIND) $(VG_FLAGS) --log-file=valgrind_3p_mixed_view_%p.log \
		$(BIN_DIR)/master -w 18 -h 18 -d 120 -t 5 -s 202 -v $(BIN_DIR)/view -p $(BIN_DIR)/greedy $(BIN_DIR)/cluster $(BIN_DIR)/greedy
...
valgrind-maxp-9-noview: all
	...
	$(VALGRIND) $(VG_FLAGS) --log-file=valgrind_max_players_%p.log \
		...
		$(BIN_DIR)/greedy $(BIN_DIR)/cluster $(BIN_DIR)/greedy \
		$(BIN_DIR)/greedy $(BIN_DIR)/cluster $(BIN_DIR)/greedy \
		$(BIN_DIR)/greedy $(BIN_DIR)/cluster $(BIN_DIR)/greedy
...
valgrind-large-board-50x50-2p-noview: all
	...
	$(VALGRIND) $(VG_FLAGS) --log-file=valgrind_large_board_%p.log \
		$(BIN_DIR)/master -w 50 -h 50 -d 1 -t 8 -s 400 -p $(BIN_DIR)/greedy $(BIN_DIR)/cluster
...
valgrind-timeout-fast-2p-view: all
	...
	TERM=$(TERM) $(VALGRIND) $(VG_FLAGS) --log-file=valgrind_timeout_view_%p.log \
		$(BIN_DIR)/master -w 10 -h 10 -d 50 -t 1 -s 500 -v $(BIN_DIR)/view -p $(BIN_DIR)/greedy $(BIN_DIR)/cluster
...
valgrind-timeout-fast-2p-noview: all
	...
	$(VALGRIND) $(VG_FLAGS) --log-file=valgrind_timeout_%p.log \
		$(BIN_DIR)/master -w 10 -h 10 -d 1 -t 1 -s 501 -p $(BIN_DIR)/greedy $(BIN_DIR)/cluster
...
valgrind-delay-high-view: all
	...
	TERM=$(TERM) $(VALGRIND) $(VG_FLAGS) --log-file=valgrind_high_delay_%p.log \
		$(BIN_DIR)/master -w 12 -h 12 -d 500 -t 6 -s 600 -v $(BIN_DIR)/view -p $(BIN_DIR)/greedy
